...
def income_category_summary(request):
    ...
    def get_income_category_amount(source):
        amount = 0
        filtered_by_category = incomes.filter(category=source)
...
def statsi_view(request):
    todays_date = datetime.date.today()
    incomes = UserIncome.objects.filter(owner=request.user, date__lte=todays_date)
    ...
    # Pass the income data to the template
    context = {
        'income_data': income_data,
    }

    return render(request, 'income/statsi.html', context)

def export_csvi(request):
    response = HttpResponse(content_type='text/csv') 
    response['Content-Disposition'] = 'attachment; filename=Incomes' + datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S') + '.csv'
